...
onButtonClick(event) {
  event.preventDefault();
  const before = parseInt(this.input.value, 10) || 0;
  ...
  const after = parseInt(this.input.value, 10) || 0;

  if (after === 0) {
   $(this).parents('td.cart-item__quantity').find('cart-remove-button').trigger('click');
    return;              
  }
...
onCloseButtonClick(event) {
  // Stop handler ONLY for .mobile--drawer--search context
  if (event.currentTarget.closest('.mobile--drawer--search')) {
    return; // Prevents submenu closing only for that specific context
  }
